...
authRouter.post("/register", async (req: Request, res: Response) => {
    ...
    const passwordHash = await fromPromise(hashPassword(password), 
    (err) => new Error("Password hashing error"));

    if(passwordHash.isErr()) {
        return res.status(500).json({error: `Failed to hash password: ${passwordHash.error.message}`});
    }
